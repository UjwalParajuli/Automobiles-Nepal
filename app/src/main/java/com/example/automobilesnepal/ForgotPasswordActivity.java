...
public class ForgotPasswordActivity extends AppCompatActivity {
    private EditText edit_text_forgot_password, edit_text_forgot_password_email;
    ...
    protected void onCreate(Bundle savedInstanceState) {
        ...
        edit_text_forgot_password_email = findViewById(R.id.edit_text_forgot_password_email);
        ...
        edit_text_forgot_password.addTextChangedListener(passwordTextWatcher);
    ...
    private TextWatcher passwordTextWatcher = new TextWatcher() {
        ...
        public void onTextChanged(CharSequence s, int start, int before, int count) {
            String pass = edit_text_forgot_password.getText().toString().trim();
            if (!PASSWORD_PATTERN.matcher(pass).matches()){
                edit_text_forgot_password.setError("Password must contain one uppercase, lowercase, digit, special character and must be 6 character long");
                button_forgot_password.setEnabled(false);

            }
            else {
                edit_text_forgot_password.setError(null);
    ...
    private void forgotPassword(){
        ...
        boolean error = false;
        String url = "http://192.168.1.65:81/android/forgot_password.php";
        email = edit_text_forgot_password_email.getText().toString().trim();
        password = edit_text_forgot_password.getText().toString().trim();

        if (email.isEmpty()){
            edit_text_forgot_password_email.setError("Please fill this field");
            error = true;
        }
        if (password.isEmpty()){
            edit_text_forgot_password.setError("Please fill this field");
            error = true;
        }
        if (!PASSWORD_PATTERN.matcher(password).matches()) {
            edit_text_forgot_password.setError("Password too weak. Must be 6 character long");
            error = true;
        }
